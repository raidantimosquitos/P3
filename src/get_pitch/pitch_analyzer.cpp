...
namespace upc {
 ...
 /// \DONE Segundo máximo de la autocorrelación calculado.
    ...
    if(!os.good()) {
      cerr << "Error reading output file " << "r1norm.txt" << " (" << strerror(errno) << ")\n";
      return -3;
    }
    ...
    if (!os1.good()) {
      cerr << "Error reading output file " << "rmaxnorm.txt" << " (" << strerror(errno) << ")\n";
      return -3;
    }
